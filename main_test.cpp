...
TEST(sorting_extra, largest_child)
{
    const vector<Track>& HEAP = {
        make_track(7),
        make_track(3), make_track(5),
        make_track(2), make_track(1), make_track(1), make_track(2)
    };
    ...
    EXPECT_FALSE(largest_child(HEAP, 0, 0, child, which));
    EXPECT_FALSE(largest_child(HEAP, 2, 4, child, which));
    EXPECT_TRUE(largest_child(HEAP, 0, 1, child, which));
    EXPECT_EQ(which, 1);
    EXPECT_TRUE(largest_child(HEAP, 0, 2, child, which));
    EXPECT_EQ(which, 2);
    EXPECT_TRUE(largest_child(HEAP, 1, 3, child, which));
    EXPECT_EQ(which, 3);
    EXPECT_TRUE(largest_child(HEAP, 1, 4, child, which));
    EXPECT_EQ(which, 3);
}
